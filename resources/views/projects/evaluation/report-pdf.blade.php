...
<body>
        ...
        <table class="header-table">
            <tr>
                ...
                <td class="header-meta">
                    ...
                    <span class="meta-label" style="margin-top: 8px;">Programa</span>
                    {{ $committeeLeader->cityProgram->program->name ?? 'No disponible' }}
                </td>
            </tr>
        </table>
    </div>
    ...
    <h2 class="page-title">Reporte de evaluacion de proyectos</h2>
    <p class="page-description">
        Resumen ejecutivo del proceso de evaluacion del comite para el programa
    ...
    <div class="context-box page-break-avoid">
        <h3 class="section-title">Contexto del reporte</h3>
        <span class="chip">Rol: Committee leader</span>
        <span class="chip">Programa: {{ $committeeLeader->cityProgram->program->name ?? 'No disponible' }}</span>
        @if (! empty($committeeLeader->cityProgram->city->name))
            <span class="chip">Ciudad: {{ $committeeLeader->cityProgram->city->name }}</span>
        @endif
    ...
    @foreach ($reportVisuals as $visual)
        @php
            ...
            $visualMaxValue = max(array_column($visualSegments, 'value') ?: [0]);
        @endphp
                ...
                <tr>
                    <td>
                        <div class="mini-metric">
                            ...
                            <div class="metric-value">{{ $visual['total_value'] }}</div>
                        </div>
                    </td>
                    <td>
                        <div class="mini-metric">
                            <div class="metric-label">Categorias</div>
                            <div class="metric-value">{{ count($visualSegments) }}</div>
                        </div>
                    </td>
                    <td>
                        <div class="mini-metric">
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 30%;">Representacion</th>
                ...
                <tbody>
                    @foreach ($visualSegments as $segment)
                        ...
                        <tr>
                            ...
                            <td>{{ $segment['description'] ?? 'Resultado consolidado del comite.' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
